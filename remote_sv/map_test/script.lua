-- ============================================================================
--  VERSION-CHECK WORKAROUND (semver-aware) вЂ” injected ahead of the map logic.
--  The baked sv_loader warns whenever  installed ~= GitHub  (BOTH directions),
--  so a forced portal version-bump (installed > GitHub) shows a FALSE
--  "update available". We intercept the baked version-file parse: when GitHub
--  is NOT newer than what's installed, we hand the check the installed version
--  so it sees equality and stays silent. A genuinely newer GitHub still warns.
-- ============================================================================
do
    ...
    string.match = function(s, pat, ...)
        if pat == "([^\n]+)\n([^\n]+)" then            -- the baked version-file parse
            local githubV, changelog = _match(s, pat, ...)
            if githubV ~= nil then
                local localV = GetResourceMetadata(GetCurrentResourceName(), "version", 0)
                if parseVer(githubV) <= parseVer(localV) then
                    if Debug then
                        print("^3[version] workaround: installed " .. tostring(localV) ..
                              " >= github " .. tostring(githubV) .. " -> staying silent^7")
                    end
                    return localV, changelog           -- force equality -> baked check silent
                end
            end
            return githubV, changelog                  -- github newer (or bad) -> let it warn
        end
        return _match(s, pat, ...)
    end
end
